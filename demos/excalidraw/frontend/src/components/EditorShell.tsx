...
import { loadStoredExcalidrawScene, saveStoredExcalidrawScene } from '../dionysys/session';
import {
  createAppliedDecision,
  getFeedbackStorageKey,
  type AppliedDecisionPayload,
} from '../dionysys/excalidrawDecisions';
...
interface EditorShellProps {
  client: Pick<DionysysClient, 'events' | 'feedback' | 'sessions'>;
...
export function EditorShell({ client, adaptiveMode, persistenceMode, sessionId, onAdaptiveModeChange, browserId, onOpenAdmin }: EditorShellProps) {
  ...
  useEffect(() => {
    ...
    eventCollector.setClient(client);
  }, [client, sessionId]);
  ...
  useEffect(() => {
    ...
    const decision = createAppliedDecision({
      mode: adaptiveMode,
    ...
    if (lastAppliedDecisionKeyRef.current === decision.decisionKey) return;
    lastAppliedDecisionKeyRef.current = decision.decisionKey;
    ...
    setShowFeedbackPrompt(window.sessionStorage.getItem(getFeedbackStorageKey(sessionId, decision.decisionKey)) !== 'true');
  }, [adaptiveMode, currentVariant, currentPersonality, decisionConfidence, lastDecision?.actionId, sessionId]);
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      ...
      navigator.sendBeacon(
        client.sessions.getCompleteUrl(sessionId),
        new Blob([JSON.stringify(body)], { type: 'application/json' })
      );
    };
    window.addEventListener('beforeunload', handleBeforeUnload);
    return () => window.removeEventListener('beforeunload', handleBeforeUnload);
  }, [client, sessionId, browserId]);
