...
The Dionysys Adaptive UI framework is designed to abstract away the complex logic of identifying user personas from behavioral signals (telemetry) and determining which UI variants to show them (contextual bandit policies).

The architecture is divided into two primary packages to maintain strict separation of concerns:

## 1. `@dionysys/core` (The Brain)
